...
function ToolsPageContent() {
  const getSlug = (name: string) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-')
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {category.tools.map((tool, toolIndex) => (
                    <motion.div
                      ...
                      transition={{ 
                        duration: 0.4,
                        delay: (categoryIndex * category.tools.length + toolIndex) * 0.1 
                      }}
                    >
                      <Link 
                        href={`/tools/${getSlug(category.name)}/${getSlug(tool.name)}`}
                          ...
                          <CardContent className="p-6">
                            <div className="space-y-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {tool.popularTools.slice(0, 3).map((popularTool) => (
                                  <span
                                    key={popularTool}
