...
import { generatePassword, calculatePasswordStrength, PasswordOptions } from "@/lib/utils/password"
...
const aboutContent = (
  <div className="space-y-6">
    <p>
      ...
      Generate strong, random passwords with customizable options for maximum security.
    </p>
    ...
    <div className="space-y-4">
      <div>
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li><strong>Length Control:</strong> Choose password length (8-128 characters)</li>
          <li><strong>Exclusions:</strong> Avoid similar/ambiguous characters</li>
          <li><strong>Memorable Mode:</strong> Generate pronounceable passwords</li>
          <li><strong>Pattern Mode:</strong> Create passwords matching specific patterns</li>
        </ul>
      </div>
      ...
      <div>
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li><strong>One-Click Copy:</strong> Copy passwords to clipboard</li>
          <li><strong>Secure Generation:</strong> Cryptographically secure random numbers</li>
          <li><strong>Multiple Formats:</strong> Various password styles and patterns</li>
        </ul>
      </div>
    </div>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-medium">Password Strength: {getStrengthLabel(strength)}</span>
                    <span>{strength}%</span>
                  </div>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="text-base">Password Length: {options.length} characters</Label>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            setOptions(prev => ({ ...prev, memorable: checked, pattern: '' }))
                          }
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        <Label htmlFor="similar">Allow Similar Characters</Label>
                        <Switch
                          id="similar"
                          checked={options.similar}
                          onCheckedChange={(checked) => 
                            setOptions(prev => ({ ...prev, similar: checked }))
                          }
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        <Label htmlFor="ambiguous">Allow Ambiguous Characters</Label>
                        <Switch
                          id="ambiguous"
                          checked={options.ambiguous}
                          onCheckedChange={(checked) => 
                            setOptions(prev => ({ ...prev, ambiguous: checked }))
                          }
                        />
                      </div>
                    </div>
              ...
              <Card className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                  <div className={options.uppercase ? "text-green-500" : "text-red-500"}>
                    {options.uppercase ? "✓" : "✗"} Includes uppercase letters
                  </div>
                  <div className={options.lowercase ? "text-green-500" : "text-red-500"}>
                    {options.lowercase ? "✓" : "✗"} Includes lowercase letters
                  </div>
                  <div className={options.numbers ? "text-green-500" : "text-red-500"}>
                    {options.numbers ? "✓" : "✗"} Includes numbers
                  </div>
                  <div className={options.symbols ? "text-green-500" : "text-red-500"}>
                    {options.symbols ? "✓" : "✗"} Includes symbols
                  </div>
                  <div className={options.length >= 12 ? "text-green-500" : "text-red-500"}>
                    {options.length >= 12 ? "✓" : "✗"} Length ≥ 12 characters
                  </div>
                  <div className={!options.ambiguous ? "text-green-500" : "text-red-500"}>
